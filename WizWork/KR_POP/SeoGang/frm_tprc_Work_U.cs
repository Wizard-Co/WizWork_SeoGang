...
namespace WizWork
{
    public partial class frm_tprc_Work_U : Form
    {
        ...
        private List<string> MoldList = new List<string>();   //2022-04-18 금형 담는 리스트
        ...
        private bool SaveData()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < lstWork.Count; i++)
                {
                    ...
                    if (MoldList.Count > 0)
                    {
                        for (int x = 0; x < MoldList.Count(); x++)
                        {
                            Dictionary<string, object> sqlParameter6 = new Dictionary<string, object>();

                            if (i == 0)
                            {
                                sqlParameter6.Add("JobID", ConvertDouble(lstWork[i].JobID));
                            }
                            else
                            {
                            ...
                            sqlParameter6.Add("MoldID", string.IsNullOrEmpty(MoldList[x].ToString()) ? "" : MoldList[x].ToString());   //old : 금형 한개만 쓸 경우. Frm_tprc_Main.list_tMold[0].sMoldID
                            sqlParameter6.Add("RealCavity", 0);
                            sqlParameter6.Add("HitCount", lstWork[i].WorkQty);
                            sqlParameter6.Add("CreateUserID", Frm_tprc_Main.g_tBase.PersonID);
        ...
        private void WorkMoldID()
        {
            try
            {
                ...
                Dictionary<string, object> sqlParameter = new Dictionary<string, object>();
                sqlParameter.Add("JobID", int.Parse(updateJobID));
                DataTable dt = DataStore.Instance.ProcedureToDataTable("xp_Work_sMoldID", sqlParameter, false);

                if (dt != null && dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        MoldList.Add(dr["MoldID"].ToString()); //dr에 있는 moldid 리스트에 담아버림.
                    }
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("금형 조회 :" + ex.ToString());
            }
        }
        ...
        private bool CheckMachine()
        {
            ...
            try
            {
                ...
                sqlParameter.Add("MachineID", Frm_tprc_Main.g_tBase.MachineID);
                DataSet ds = null;
                ds = DataStore.Instance.ProcedureToDataSet("xp_WizWork_sCheckMcReqularInspect", sqlParameter, false);
                if (ds != null && ds.Tables[0].Rows.Count > 0)
                {
                    flag = true;
                    return flag;
            ...
            {
                WizCommon.Popup.MyMessageBox.ShowBox(string.Format("오류! 관리자에게 문의<CheckMachine>\r\n{0}", Exception.Message), "[오류]", 0, 1);
                return flag;
            }
        }
