...
namespace WizIns.PopUp
{
    public partial class Frm_CMKeypad : Form
    {
        ...
        WizWork.Lib.MES_WTS_Lib WTS_Lib = new WizWork.Lib.MES_WTS_Lib();

        public Frm_CMKeypad(string strTitle, string strValue)
        {
            InitializeComponent();
            this.Text = strTitle;
            this.button1.Text = strTitle;
            KeypadStr = "";
                ...
                if (Owner.Name != null)
                {
                    ...
                    if (this.Owner.Name.ToString() == "Frm_tprc_PlanInput_Q")
                    {
                        if (strKobType == "OK")
                        {
                            ...
                            if (this.button1.Text == "LOTID")
                            {
                                if (WTS_Lib.GetCheckLoID(this.tbInputText.Text) == false)
                                {

                                    MessageBox.Show("잘못된 LOTID호입니다", "입력번호", MessageBoxButtons.OK, MessageBoxIcon.Information);
